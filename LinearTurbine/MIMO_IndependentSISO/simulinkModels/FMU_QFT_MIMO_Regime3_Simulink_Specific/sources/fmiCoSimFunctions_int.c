													...
													const  FMIReal value[])
{
	...
	util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s is not suported with inline integration",label);
	return FMIWarning;
#else
	size_t nu = comp->nIn; 
	size_t i;
	...
	for (i = 0; i < nvr; i++) {
		...
		if (order[i] == 1) {
			...
			case dsInput:
				...
				break;
			default:
				util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: variable is not input", label);
				return FMIWarning;
			}
		} else {
			util_logger(comp, comp->instanceName, FMIWarning, loggBadCall,
													 ...
													 FMIReal value[])
{
	...
	Component* comp = (Component*) c;
#ifdef ONLY_INCLUDE_INLINE_INTEGRATION
	util_logger(comp, comp->instanceName, loggBadCall, loggBadCall, "%s is not suported with inline integration",label);
	return FMIWarning;
#else
	...
	for (i = 0; i < nvr; i++) {
		...
		if (order[i] == 1) {
			...
			case dsOutput:
				...
				break;
			default:
				util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: variable is not output", label);
				return FMIWarning;
				break;
			}
		} else if (order[i] > 1) {
			...
			util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: derivative order 0 is not allowed", label);
			return FMIWarning;
		}
	}
	util_logger(comp, comp->instanceName, FMIOK, loggFuncCall, "%s", label);
...
#endif
{
	...
	if (comp->mStatus == modelTerminated || comp->terminationByModel == FMITrue) {
		util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: model is terminated",label);
		return FMIWarning;
	}
...

{
	...
	if(comp->mStatus != modelContinousTimeMode){
		if(comp->mStatus == modelEventModeExit){
			...
			util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: more eventIteration needed before calling hybridDoStep",label);
			return FMIWarning;
		}
	}
	/* check if reasonably close to expected time */
