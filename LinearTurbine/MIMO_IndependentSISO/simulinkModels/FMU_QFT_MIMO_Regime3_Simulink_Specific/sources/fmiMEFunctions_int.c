...
DYMOLA_STATIC_IMPORT size_t dyn_instanceDataSize;
...
#if defined(FMI_2) && !(defined(NO_FILE) || defined(RTTWINCAT))
...
DYMOLA_STATIC FMIStatus fmiSetTime_(FMIComponent c, FMIReal time)
{
	...
	if (comp->mStatus == modelEventModeExit) {
		if (comp->nStates != 0) {
			util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: only allowed for discrete models when not in continuous time mode", label);
			return FMIWarning;
		}
		...
		util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: not allowed in this state", label);
		return FMIWarning;
	}
#endif
	comp->time = time;
...
{	
	...
	switch (comp->mStatus) {
		...
		case modelEventMode2:
			...
			break;
		default:
			util_logger(comp, comp->instanceName, FMIWarning, loggBadCall, "%s: may only be called in event mode", label);
			return FMIWarning;
	}
	comp->icall = 0;
	QiErr = util_refresh_cache(comp, iDemandEventHandling, NULL, &converged);
...
DYMOLA_STATIC FMIStatus fmiSetClock_(FMIComponent c, const FMIInteger clockIndex[], size_t nClockIndex, const FMIBoolean tick[], const FMIBoolean subactive[])
{
	...
	for( i = 0; i < nClockIndex; ++i){
		if(clockIndex[i] > (int) FMIClockValueReferences_[0] || clockIndex[i] < 1){
			...
			if(status != FMIOK){
				util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "%s Error when setting clock &d", label, clockIndex[i]);
